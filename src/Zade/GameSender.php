...
class GameSender extends PluginTask {
	...
	{
		$puntos = new puntos($this->plugin->getDataFolder() . "/puntos.yml", puntos::YAML);
                $slots = new puntos($this->plugin->getDataFolder() . "/slots.yml", puntos::YAML);
		$arenas = $puntos->get("arenas");
		if(!empty($arenas))
		{
			foreach($arenas as $arena)
			{
				$time = $puntos->get($arena . "PlayTime");
				$timeToStart = $puntos->get($arena . "StartTime");
				...
				if($levelArena instanceof Level)
				{
					...
					{
						$puntos->set($arena . "PlayTime", 470);
						$puntos->set($arena . "StartTime", 30);
                                                $puntos->set($arena . "start", 0);
					}
					else
					{
                                                if(count($playersArena)>=6)
                                                {
                                                    $puntos->set($arena . "start", 1);
                                                    $puntos->save();
                                                }
						if($puntos->get($arena . "start")==1)
						{
							if($timeToStart>0)
							{
								$timeToStart--;
								foreach($playersArena as $pl)
								{
									$pl->sendTip(TE::WHITE."Game starts in ".TE::GREEN . $timeToStart . TE::RESET ."");
                                                                        ...
                                                                        $temas = array_rand($puntos->get("temas"));
                                                                        $tema = $puntos->get("temas")[$temas];
                                                                        $puntos->set($arena . "tema", $tema);
                                                                        }
								}
                                                                if($timeToStart==29)
                                                                {
							...
							else
							{
								...
								if($time == 469)
								{
                                                                    $laimit = new puntos($this->plugin->getDataFolder() . "/limit.yml", puntos::YAML);
									foreach($playersArena as $pl)
									{
										...
										foreach($playersArena as $pl)
										{
											$pl->sendMessage($this->prefix .TE::YELLOW. $time2 . " " .TE::GREEN. "seconds left...");
                                                                ...
                                                                else
                                                                {
                                                                    ...
                                                                    {
                                                                        $points = new puntos($this->plugin->getDataFolder() . "/puntos".$arena.".yml", puntos::YAML);
                                                                        foreach ($playersArena as $pl)
                                                                        {
                                                                            $yo = $puntos->get("actual".$arena);
                                                                            if($yo!=$pl->getName())
                                                                            {
                                                                                $pts = $points->get($yo);
                                                                                ...
                                                                                $tot = $pts + $puntos;
                                                                                $points->set($puntos->get("actual".$arena), $tot);
                                                                                $points->save();
                                                                    ...
                                                                    if($time==170)
                                                                    {
                                                                        ...
                                                                        foreach($playersArena as $pl)
                                                                        {
                                                                            ...
                                                                            $pl->getInventory()->setHotbarSlotIndex(1,1);
                                                                            $pl->getInventory()->setHotbarSlotIndex(2,2);
                                                                            $pl->getInventory()->setHotbarSlotIndex(3,3);
                                                                            $pl->getInventory()->setHotbarSlotIndex(4,4);
                                                                            $pl->getInventory()->setHotbarSlotIndex(5,5);
                                                                        ...
                                                                        if($slots->get("slot1".$arena)!=null)
                                                                        {
                                                                            ...
                                                                            $puntos->set("actual".$arena, $actual);
                                                                            $thespawn = $puntos->get($arena . "Spawn1");
                                                                            foreach($playersArena as $pl)
                                                                            {
                                                                                $rand = mt_rand(1, 2);
                                                                    ...
                                                                    {
                                                                        $points = new puntos($this->plugin->getDataFolder() . "/puntos".$arena.".yml", puntos::YAML);
                                                                        $limit = new puntos($this->plugin->getDataFolder() . "/limit.yml", puntos::YAML);
								...
								$puntos->set($arena . "PlayTime", $time);
							}
						}
						else
						{
